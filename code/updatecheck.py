...
__author__ = 'The real barremans'
__copyright__ = 'Copyright (C) 2023, Barre'
__credits__ = ['Victor Santiago']
__license__ = 'GNU License'
__version__ = '0.0.1'
__maintainer__ = 'Barre'
            ...
            try:
                ...
                if current_version < online_version:
                    ...
                    mb1 = messagebox.askyesno(
                        'Update!', f'{_AppName_} {__version__} needs to update to version {online_version_str}')
                    if mb1 is True:
                        # Replace the URL with the actual update file URL
                        update_file_url = 'https://github.com/vsantiago113/Tkinter-MyTestApp/raw/master/updates/MyTestApp.msi?raw=true'
...
# updates/setup CGK TOOLS.msi
        ...
        def start_update_manager():
            ...
            try:
                ...
                file_path = os.path.join(download_folder, f'{_AppName_}.msi')
                with requests.get('https://github.com/barremans/cgk-tools/blob/main/updates/setup%20CGK%20TOOLS.msi?raw=true', stream=True) as r:
                    self.progressbar['maximum'] = int(
                        r.headers.get('Content-Length'))
                    r.raise_for_status()
